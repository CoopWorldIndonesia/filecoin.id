...
class M_user extends CI_Model
{
    ...
    public function get_bonus_global($id)
    {
        return $this->db->select('bonus_global.id, user.username, bonus_global.mtm, bonus_global.level_fm, bonus_global.datecreate')
            ->from('bonus_global')
            ->join('user', 'bonus_global.user_id = user.id')
            ->where('bonus_global.user_id', $id)
            ->where('mtm !=', 0)
    ...
    {
        return $this->db->select('deposit.id, deposit.datecreate, user.username, deposit.coin, deposit.txid, deposit.type_coin, deposit.note, deposit.is_confirm')
            ->from('deposit')
